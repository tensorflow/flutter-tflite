...
image_lib.Image copyMakeBorder(
  image_lib.Image src,
  int top,
  int bottom,
  int left,
  int right,
  image_lib.Color borderColor,
) {
  final targetWidth = src.width + left + right;
  final targetHeight = src.height + top + bottom;

  var targetImage = image_lib.Image(
    width: targetWidth,
    height: targetHeight,
  );

  targetImage = image_lib.fill(
    targetImage,
    color: borderColor,
  );
  ...
  for (var y = 0; y < src.height; y++) {
    for (var x = 0; x < src.width; x++) {
      ...
      targetImage.setPixel(x + left, y + top, color);
    }
  }
  return targetImage;
}
...
Future<Uint8List> loadImage(String path) async {
  ...
  final image_lib.Image image = image_lib.decodeJpg(data)!;
  final image_lib.Image resized = image_lib.copyResize(image, width: 100);
  final List<int> resizedBytes = image_lib.encodeJpg(resized);
